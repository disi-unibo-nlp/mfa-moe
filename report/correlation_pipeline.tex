...
For subsequent runs, these per-trace statistics are reduced during the
teacher-forced forward pass rather than reconstructed from serialized raw
activations.  The default artifact retains compact scalar features and top-$k$
expert identifiers, while full hidden-state and router tensors are available
only for a small explicit audit run.  Each trace records projected raw bytes
and actual persisted bytes.  This storage change preserves the existing
features and adds normalized router confidence, selected probability mass,
the $p_k-p_{k+1}$ boundary margin, and the mean top-$k$ versus non-top-$k$
probability gap.
Entropy and normalized confidence are not both entered as correlation evidence:
new analyses report the confidence orientation, which is an affine transform of
normalized entropy and therefore contains the same Pearson information.
