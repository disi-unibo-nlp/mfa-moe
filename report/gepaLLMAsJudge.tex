...
The Qwen3.6 final fit uses seed 42, the 21-example context-aware few-shot
...
The final alignment uses Qwen3.8-27B
...
confusion.  The appropriate conclusion is that the
...
\paragraph{Frozen-prompt deployment and reasoning effort.}
The NVFP4 deployment comparison evaluates the frozen 40-example prompt
on the same 407 development-validation units, with temperature zero,
a 4,096-token budget, seed 42, one worker, and one uncached pass.
Table~\ref{tab:judge-effort} reports the saved outputs. Both settings have
100\% valid-output coverage and no request errors. They disagree on 41/407
labels (10.1\%): identical aggregate accuracy does not imply interchangeable
annotations. This is exploratory evidence on reused validation responses,
not independent validation of labels on generated benchmark traces.
\input{judge_effort_table}

The 336-unit partition is excluded from these final fits. Because its
responses have been inspected in preliminary work, it should not be
described as genuinely untouched; an independent generalization estimate
requires unseen responses.
