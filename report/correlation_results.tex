...
The Qwen numerical results use the run
...
metrics include final-answer tokens. The \texttt{full/reasoning} view
...
\input{correlation_overview_tables}
...
($-0.727$--$-0.238$). Attempts within AIME/AMC problems are dependent; naive
...
are removed from the report's outcome panels, to keep the outcome panels focused on model-assigned classes.
They are not the seven predicted reasoning classes. Detector outputs remain in the source artifacts.

\subsection{Repeated-attempt findings}

There are 100 eligible complete avg@32 groups: 30 AIME24, 30 AIME25,
and 40 AMC23 problems. Problem means use all declared attempts, including
limit hits, and binary outcomes are complete. The run supports both between-problem
rank associations and within-problem correct/incorrect contrasts.

\input{correlation_repeated_tables}
...
\input{correlation_expert_table}
...
whole problems. Separate held-out guiding results are reported in
Section~\ref{sec:guiding}.
