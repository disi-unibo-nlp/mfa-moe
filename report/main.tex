...
should not be called validated transfer accuracy.
...
Correlation pipeline & Smoke validated; full study pending & The named \texttt{correlation\_pipeline} defaults to Unsloth Qwen3.5-35B-A3B.  Generation uses the \texttt{UD-Q4\_K\_XL} GGUF with embedded native-MTP speculative weights; replay quantizes the matching BF16 Transformers checkpoint to 4-bit with Unsloth \texttt{FastModel}, skips the unused vision tower and CausalLM auxiliary loss, disables KV caching, and retains router and hidden-state outputs.  On 2026-08-20 the complete generation--replay--analysis command succeeded for one MATH500, one AIME24, and one Minerva trace, extracting all 40 layers over 8,196, 6,575, and 7,712 generated tokens, respectively.  This operational smoke test is not statistical evidence: the full eight-SPIRAL-plus-repository benchmark execution and inferential analysis remain pending.  Generation and replay use different 4-bit formats. \\
