...
static-reward baselines optimize a Qwen3.6-27B prompt for sentence-level
Schoenfeld episode classification, while a new LLM-as-a-judge reward
configuration awaits rerunning.  This judge experiment evaluates annotation
capability and is not evidence about Qwen router dynamics.  Experiment~0b is
...
\paragraph{New informative reward (implementation complete; run pending).}
The current implementation replaces the default static GEPA reward with an
...
independent selection and reporting criteria.  The judge currently runs
deterministically with model reasoning disabled; reasoning-enabled evaluation
is deferred to a 5090 run because it substantially increases inference time.
Every judge call is persisted as JSONL with its component scores, normalized
score, and free-text explanation, permitting a direct audit of feedback
quality.  The pending rerun uses GEPA's heavy budget and a 4096-token
reflection cap while retaining the existing one-GPU, 64\,GB, 48-hour SLURM
allocation.
All numerical results below precede this change and therefore remain
static-reward baselines rather than evidence for the new judge.
...
macro-F1 selection with the light GEPA budget.}
...
are therefore selection-optimistic.}
...
\begin{table*}[t]
\centering
\caption{Argmax inspection-label counts on unannotated external reasoning
units.  These are prediction frequencies, not accuracies; no compatible
Schoenfeld labels are available for these examples.}
...
Monitor; Explore is never selected, 12.7\% of units activate no probe, and no
external gold episode labels exist.  ProcessBench Verify predictions identify
review-heavy passages after errors but do not identify correctness.
...
external argmax outputs have no episode ground truth and should not be called
transfer accuracy.
...
multiple labels; and a manually labelled, benchmark-stratified sample is
required before reporting transfer accuracy.  It would also be useful to
...
likewise favors conservative conclusions: context-aware light-budget GEPA with
macro-F1 selection reaches 0.650 cross-validated macro-F1, but its improvement
over the prior selector is small, a heavy budget is worse, and Monitor
overprediction remains substantial.  The separate Qwen3.5 boundary probes
...
setup/execution/review taxonomy.  They therefore support prospective
...
GEPA episode judge (Experiment 0a) & CV complete; validation pending & Context-aware five-fold grouped CV over 2,789 units gives 67.34\% accuracy and 65.05\% macro-F1 for light-budget GEPA with macro-F1 selection. Heavy is worse (66.69\%, 64.37\%). Review 180 flagged units, constrain decoding, evaluate seed and optimized prompts on every outer fold, and designate a genuinely unseen test set. \\
Schoenfeld boundary probes (Experiment 0b) & Done & Qwen3.5 GPTQ probes cover 3,087 units, seven targets, and 41 hidden-state indices; selected-layer AUROC is 0.835--0.934.  External inspection covers 581 units but has no episode ground truth and never selects Explore.  Grouped CV remains pending; add nested layer selection, common multiclass calibration, robust segmentation, and a manually labelled transfer set. \\
