...
if [[ "$DRY_RUN" != true ]]; then
    ...
    mkdir -p "$HF_CACHE_DIR" "$PHYS_DIR/results/moe_margin_guiding"
    for writable_dir in "$HF_CACHE_DIR" "$PHYS_DIR/results/moe_margin_guiding"; do
...
FULL_COMMAND=("${DOCKER_ARGS[@]}" --entrypoint python3 "$IMAGE_NAME"
    -m moe_exp.moe_margin_guiding.run "$@")

echo "MoE margin guiding: image=$IMAGE_NAME, workspace=$PHYS_DIR, HF_HOME=$HF_CACHE_DIR"
if [[ "$DRY_RUN" == true ]]; then
