...
if [[ $# -lt 1 ]]; then
    echo "Usage: $0 <prepare|prepare-global|prepare-sampling|fit|generate|compare> [arguments...]" >&2
    exit 2
fi
case "$1" in
    prepare|prepare-global|prepare-sampling|fit|generate|compare) ;;
    *) echo "Unknown command: $1 (choose prepare, fit, generate or compare)" >&2; exit 2 ;;
esac
...
if [[ "$DRY_RUN" != true ]]; then
    ...
    mkdir -p "$HF_CACHE_DIR" "$PHYS_DIR/results/moe_identity_guiding"
    for writable_dir in "$HF_CACHE_DIR" "$PHYS_DIR/results/moe_identity_guiding"; do
...
FULL_COMMAND=("${DOCKER_ARGS[@]}" --entrypoint python3 "$IMAGE_NAME"
    -m moe_exp.moe_identity_guiding.run "$@")

echo "MoE identity guiding: image=$IMAGE_NAME, workspace=$PHYS_DIR, HF_HOME=$HF_CACHE_DIR"
if [[ "$DRY_RUN" == true ]]; then
