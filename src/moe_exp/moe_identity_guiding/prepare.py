    ...
    for path in paths:
        for row in iter_jsonl(path):
            if type(row.get("is_correct")) is not bool:
                excluded["unscored"] += 1
                continue
            if not (row.get("model_logs") or {}).get("selected_experts"):
...
def prepare_global(generation_paths, routing_paths, output_dir: Path,
    ...
    for path in routing_paths:
        for row in iter_jsonl(path):
            ...
            if not logs.get("selected_experts") or not Path(logs["selected_experts"]).is_file():
                exclusions["missing_calibration_tensor"] += 1
                continue
            calibration.append({name: row.get(name) for name in (
                "dataset", "problem_id", "source_problem_id", "sample_id", "model_id",
                "is_correct", "model_logs")})
    ...
    manifest = dict(seed=seed, calibration_fraction=calibration_fraction,
                    policy_scope="global_problem_balanced", generation_models=sorted(models),
