# Historical numbered-experiment documentation

This is the pre-reorganization README retained as a historical snapshot. The
numbered packages now live under `src/moe_exp/old`, their launcher is
`old/scripts/run_pipeline.sh`, and their outputs are under `results/old`.
Use the repository-root `README.md` for the active workflows and current paths.
